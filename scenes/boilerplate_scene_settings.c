...
static void boilerplate_scene_settings_set_memory_size(VariableItem* item) {
    Boilerplate* app = variable_item_get_context(item);
    uint8_t index = variable_item_get_current_value_index(item);

    variable_item_set_current_value_text(item, memory_model[index]);
    app->mem_size = memory_capacity[index];
}
...
void boilerplate_scene_settings_on_enter(void* context) {
    ...
    // Memory model
    uint8_t size_of_mem_list = memory_get_list_size();
    item = variable_item_list_add(
        app->variable_item_list,
        "Memory:",
        size_of_mem_list,
        boilerplate_scene_settings_set_memory_size,
        app);
    value_index = value_index_uint32(app->mem_size, memory_capacity, size_of_mem_list);
    variable_item_set_current_value_index(item, value_index);
    variable_item_set_current_value_text(item, memory_model[value_index]);
